...
export function UpdatePresets(self: ModuleInstance): void {
	const presets: CompanionPresetDefinitions<ModuleSchema> = {
		...
		countdown: {
			...
			name: '剩餘秒數',
			style: {
				text: '剩餘\n$(iina-remote:remaining_seconds) 秒',
				size: 'auto',
				color: white,
				bgcolor: dark,
				show_topbar: false,
		...
		status: {
			...
			name: '播放時間',
			style: {
				text: '$(iina-remote:position_time)\n$(iina-remote:duration_time)',
				size: 'auto',
				color: white,
				bgcolor: dark,
				show_topbar: false,
	...
	self.mediaFiles.forEach((file, index) => {
		...
		presets[id] = {
			...
			style: {
				...
				size: 'auto',
				color: white,
				bgcolor: combineRgb(0, 90, 55),
				show_topbar: false,
			},
			steps: [
				{
					down: [{ actionId: 'play_media_file', options: { path: file.path, startFromBeginning: true } }],
					up: [],
				},
			],
			feedbacks: [],
